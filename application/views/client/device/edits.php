          ...
          <div class="search-field d-none d-md-block">
            <form class="d-flex align-items-center h-100" action="#">
              <div class="input-Device">
                <div class="input-Device-prepend bg-transparent">
                  <i class="input-Device-text border-0 mdi mdi-magnify"></i>
                </div>
            ...
            <div class="col-md-6 grid-margin stretch-card">
                <div class="card">
                  ...
                  <form class="forms-sample" method="post" action="">
                        ...
                        <input type="hidden" name="device" value="<?=$dserver->id_device?>">
                         <div class="form-Device row">
                        <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Device Name<br> <span style="color:red"> <?=form_error('name')?> <?php echo $this->session->flashdata('message_add_webhook_error'); ?></span></label>
                        <div class="col-sm-9">
                          <input type="text" class="form-control"  name="name" id="name" placeholder="ex : Cabang A" value="<?=$dserver->device_name?>">
                        </div>
                      </div>
                    
                      <div class="form-Device row">
                        <label for="exampleInputUsername2" class="col-sm-3 col-form-label">Server<br> <span style="color:red"> <?=form_error('deskripsi')?> <?php echo $this->session->flashdata('message_add_webhook_error'); ?></span></label>
                        <div class="col-sm-9">
                        <select name="server" id="" class="form-control">
                          <option value="">Select Server</option>
                          <?php foreach ($webhook as $key => $value) {
                          ?>
                          <option <?=($value->url == $dserver->backend_url)?"selected":""?> value="<?=$value->url?>"><?=$value->name?></option>
                          <?php   # code...
                          } ?>
                        </select>  
                      </div>
